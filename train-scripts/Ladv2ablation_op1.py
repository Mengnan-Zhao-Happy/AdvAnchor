...
def train_esd(iter_break, w_iter_break, atts, att_size,ori_flag, erase_cat,erased_index, lr,lr2, config_path, ckpt_path, diffusers_config_path, devices,ddim_steps=50):
    ...
    name = f'Ladv2ablation_op1-{erase_cat}-lr2_{lr2}-erased_{prompt}-iter_{iter_break}_{w_iter_break}-{ori_flag}-{att_size}-1'
    ...
    for i, image_name_vangogh in enumerate(images_vangogh):
        ...
        for cu_number in range(att_size):
            emb_att_set.append(model.get_learned_conditioning([f"{atts[cu_number]}'s painting"]))
            

        emb_nor = model.get_learned_conditioning([f"{prompt} painting"])
        emb_noise = model.get_learned_conditioning([f"{prompt}'s painting"], delta_prompt, ind_start, ind_end)
        emb_blank = model.get_learned_conditioning(['painting'])
        ...
        for _ in range(w_iter_break):
            ...
            e_noise_blank = (e_noise_op.to(devices[1]) - e_blank.to(devices[1]))
            with torch.no_grad():
                e_nor_blank_set = []
                if cu_len == 1:
                    e_nor = model_orig.apply_model(z_n.to(devices[1]), t_enc_ddpm.to(devices[1]), emb_nor.to(devices[1]))[0]
                    e_nor_blank = (e_nor.to(devices[1])-e_blank.to(devices[1]))
