...
def get_args():
    ...
    parser.add_argument('--model_path', default='./models/artist_model0.pth', type=str)
    ...
    parser.add_argument('--device', help='cuda device to train on', type=str, required=False, default='cuda:4')
    parser.add_argument('--normal_std', default=1, type=float, help='normal_std')
    parser.add_argument('--erase_index', type=int, default=0)
...
for i, (X, y) in enumerate(eval_loader):
    ...
    output = model(X)
    pred_cate = output.max(1)[1]-1
    if y != args.erase_index:
        eval_sum += (pred_cate == y).sum().item()
        eval_sum_n += 1
    if y == 0:
        eval_acc_0 += (pred_cate == y).sum().item()
        eval_n_0 += 1
    if y == 1:
        eval_acc_1 += (pred_cate == y).sum().item()
        eval_n_1 += 1
    if y == 2:
        eval_acc_2 += (pred_cate == y).sum().item()
        eval_n_2 += 1
    if y == 3:
        eval_acc_3 += (pred_cate == y).sum().item()
        eval_n_3 += 1
    if y == 4:
        eval_acc_4 += (pred_cate == y).sum().item()
        eval_n_4 += 1
    if y == 5:
        eval_acc_5 += (pred_cate == y).sum().item()
        eval_n_5 += 1
    if y == 6:
        eval_acc_6 += (pred_cate == y).sum().item()
        eval_n_6 += 1
    if y == 7:
        eval_acc_7 += (pred_cate == y).sum().item()
        eval_n_7 += 1
    if y == 8:
        eval_acc_8 += (pred_cate == y).sum().item()
        eval_n_8 += 1
print(eval_n_0,eval_n_1,eval_n_2,eval_n_3,eval_n_4,eval_n_5,eval_n_6,eval_n_7,eval_n_8)
print(f'test-acc0_{eval_acc_0 / eval_n_0}-acc1_{eval_acc_1 / eval_n_1}-acc2_{eval_acc_2 / eval_n_2}-acc3_{eval_acc_3 / eval_n_3}-acc4_{eval_acc_4 / eval_n_4}-acc5_{eval_acc_5 / eval_n_5}-acc6_{eval_acc_6 / eval_n_6}-acc7_{eval_acc_7 / eval_n_7}-acc8_{eval_acc_8 / eval_n_8}-acc_avg_{eval_sum / eval_sum_n}')
